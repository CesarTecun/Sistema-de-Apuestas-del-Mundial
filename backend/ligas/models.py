...
class Liga(SoftDeleteModel):
    ...
    cuota_entrada = models.DecimalField(max_digits=10, decimal_places=2, default=0, help_text='Monto que cada usuario paga al unirse a la liga')
    ...
    def actualizar_monto_total_recaudado(self):
        """Calcula y actualiza el monto total recaudado basado en participantes activos y cuota de entrada"""
        from django.db.models import Count
        participantes_activos = ParticipanteLiga.objects.filter(
            fk_id_liga=self.id_liga,
            estado_participacion='Activo'
        ).count()
        self.monto_total_recaudado = self.cuota_entrada * participantes_activos
        self.save(update_fields=['monto_total_recaudado'])
